...
Dove, come vedete, ho inserito il bottone ` <MenuIcon className="h-8 w-8" />` all'interno del componente `<Popover.Button>`.
...
> Se il pannello è vero (`open==true)` allora renderizza l'icona `<XIcon />`, altrimenti l'icona `<MenuIcon />`.
