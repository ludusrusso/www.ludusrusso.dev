...
Nei capitoli precedenti abbiamo iniziato a muoverci all'interno del progetto NextJS e iniziato a creare pagine e
...
community ReactJS, non si è trovata una tecnologia (o un gruppo di tecnologie) abbastanza orizzontale sui vari progetti, invece
esistono diversi approcci (anche molti diversi tra loro) e diverse preferenze tra gli sviluppatori.
...
il contenuto di un documento (HTML) dal modo in cui viene visualizzato (CSS). Sebbene questa idea in teoria sia molto valida,
...
in generale la development experience non è molto semplice.
...
per creare componenti. Tra le più famose ci sono [Material UI](https://mui.com/) e [React Bootstrap](https://react-bootstrap.github.io/).
...
su questo tema troviamo [`tailwindcss`](https://tailwindcss.com/), che negli ultimi anni si è distinta tantissimo grazie
...
Quindi esistono tantissime tecnologie, quale possiamo usare? Non ho una risposto a questa domanda, perché come ho detto ci sono tantissimi
...
una conoscenza di tutti gli approcci sarebbe ideale, ma ovviamente dobbiamo fare una scelta per questa guida, e dopo
averci pensato un po' credo che la cosa migliore sia continuare con l'approccio che conosco meglio, perché permetterà
...
imposizione di stile. L'utilizzo di questa libreria in combo con tailwindcss o styled component è quindi molto vantaggiosa.
...
nei prossimi, useremo principalmente TailwindUI e un po' di CSS in JS con [emotions css](https://emotion.sh/docs/introduction) quando diventa troppo complesso usare Tailwind.

### Come trarre al meglio da questa guida?

A differenza della parte di scrittura del codice, dove è abbastanza semplice spiegare il perché di certe scelte
sintattiche, la creazione e la definizione di stili grafiche è un processo che richiede molto più feedback, trial and error ed
interazione con l'applicazione. In questo capitolo sarà molto difficile per me spiegare il perché di alcune scelte
...
Questo è essenziale per il funzionamento di tailwind, perché il server tailwind monitorerà tutti i file che inseriamo qui dentro
e genererà il codice CSS in base alle classi che vogliamo usare. Quest'idea di tailwind ha un duplice vantaggio:

1. non creare codice CSS per utility class che non stiamo usando. Questo è importante perché tailwind mette a disposizione
...
di modificare questo file altrimenti potreste avere problemi con il funzionamento di tailwind.
...
un attimo per prendere familiarità con tutto (noi vederemo piano piano le utility che ci mette a disposizione).
...
- [headlessui](https://headlessui.dev/), di cui abbiamo già parlato, che ci mette a disposizione una serie di componenti
...
Riprendiamo per prima cosa il componente `Nav` che abbiamo abbozzato precedentemente, cioè il file che abbiamo chiamato `components/nav.tsx`, di cui
riporto il contenuto attuale:

```tsx
export const Nav = () => {
  return <nav>React Megatutorial Navigation</nav>;
};
```
...
La navbar dovrà contenere una lista di link posizionati in verticale, insieme ad una icona della nostra applicazione (in teoria il nostro logo
ma non avendolo useremo l'icona delle casa) che ci rimanda
...
Partiamo dalla navbar, dobbiamo farla alta 64px `h-16` (non so perché, ma è lo standard di molte navbar in giro) e di un colore
...
le icone in `@heroicons/react/24/outline`.
...
deve essere un semplice link a home), mentre la seconda in un componente `<button>`, perché appunto
...
in HTML con la props `href="/"`, ma NextJS ci mette a disposizione un componente speciale
...
Dove, come vedete, ho inserito il bottone ` <Bars3Icon className="h-8 w-8" />` all'interno del componente `<Popover.Button>`.
...
Il menu a questo punto funziona, potete provare ad aprirlo e chiederlo senza problemi, ma vi renderete subito conto che la
parte grafica non è per niente ottimale. Iniziamo a stilizzarlo meglio (in modo che almeno sia visibile) come segue:
...
Come vedete, il children del popover non è più un componente ma un render template, cioè una funzione che prende dei
...
Quando passiamo direttamente `Popover.Panel` come children al componente `Popover`, infatti,
...
> Se il pannello è vero (`open==true)` allora renderizza l'icona `<XMarkIcon />`, altrimenti l'icona `<Bars3Icon />`.

Se provate a il componente a questo punto vedrete che l'icona cambia se questo è aperto o chiuso.

#### Props del componente

A questo punto dobbiamo fare in modo da passare dall'esterno la lista di link al menu, questo è importante perché ricordiamo che dovremo renderizzare il menu in due modi diversi in base alla dimensione
...
in modalità desktop. Per farlo, dobbiamo introdurre il concetto di breakpoint in Tailwind.
...
Questi breakpoint si traducono in modificatori delle classi css che ci permettono di cambiare il layout in base
alla dimensione dello schermo. Vediamo come usarli.

L'idea di base dei modificatori di tailwind è che possiamo applicare nuovi template sovrascrivendo le classi a breakpoint diverse.
Facciamo un esempio per capire meglio: supponiamo di voler nascondere il menu del nostro header quando lo schermo è a breakpoint `md` (medium)
...
Il bottone sarà mostrano normalmente fino al breakpoint `md`, e poi verrà applicato la classe `hidden` che non lo farà più vedere.

Per fare il contrario, dobbiamo creare un componente `hidden` di base e poi mostrarlo al breakpoint `md`:

```tsx
<div className="hidden md:block">Visibile la breakpoint</div>
```
...
semplice list di `<Link />` orizzontale.
...
```tsx
export const Footer = () => {
  return <footer>React Megatutorial; all right reserved</footer>;
};
```

Il footer dovrà contenere un piccolo disclaimer con il nome del sito e l'autore e una lista di link ai social network.
...
Partiamo dal contenuto, dovremmo creare due div, la prima contenente il disclaimer mentre la seconda che contiene la lista di link social.
...
La cosa che faccio di solito è creare un componente interno dove inserire lo style utilizzando `@emotions/styled`. Dato che il componente
principale della nostra navbar è l'oggetto html `<footer>`, partirò da questo per generare l'oggetto stilizzato.
...
con styled component. Per farlo abbiamo importato la libreria `styled` da `@emtion/styled` e abbiamo
...
Facciamo lo stesso lavoro per gli altri social che vogliamo usare, nel mio caso GitHub e YouTube, in modo da avere tutto quello che ci
...
direttamente. Inoltre sto usando del codice tailwindCSS per stilizzare le icone, perché in questo caso viene molto più semplice
...
all'interno dello styled component!
...
Notare che abbiamo settato come breakpoint `768px`, il breakpoint `md` di tailwind.
...
- Come utilizzare SVG all'interno delle applicazioni React;
