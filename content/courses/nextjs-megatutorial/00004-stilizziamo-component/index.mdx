...
Con questa libreria andremo a stilizzare la navbar della nostra applicazione, cioè il file che abbiamo chiamato `components/nav.tsx`, di cui
...
La navbar dovrà contenere una lista di link posizionati in vertiale, insieme ad un titolo della nostra appliazioche che ci rimanda
...
Partiamo prima di tutto dal contenuto. Il nostro componente `<nav>` dovrà contenere un `<div>` con all'interno due componenti,
...
princiaple della nostra navbar è l'oggetto html `<nav>`, partirò da questo per generare l'oggetto stilizzato.
...
Come vedete il componente `NavStyled` non è altro che un classico componente `<nav>` HTML a cui abbiamo _appiccicato_ dello style
